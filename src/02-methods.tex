...
dormant buds positioned there. This is in tune with observations in Nature~\cite{wohlleben2016hidden}. Trees produce shots on their trunks rarely (for example when they are sick), because of the low lighting, thick bark, etc. \sk{You changed the font in Fig. 1, so it is not consistent with other figures anymore. Was that by accident?}\bb{ yes it was. Can you fix it please? I exported is a PDF so it is vectors.} 
...
also called the solution vector. \sk{If you agree, the objective function in Eq. 7 should be removed. Štefan and Damjan have tried something similar as they wrote their paper but it didn't work very well, since the tree was only trimmed, but no major cuts were performed.}\bb{ OK - go ahead and remove it.}
...
preserved during the entire lifetime of an orchard \bb{so that the tree branches do not overlap and do not compete for the same space}.
...
efficient. \bb{Can you say why - in one sentence? Was it competing inside the DDE? 
Slow? What was the problem? - If it is not clear, we may reomve the entire paragraph. The next one is a good justification.}
...
\bb{Do you have an image to show them?}

Instead of changing the objective function, we propose adding a
preprocessing level to the DDE method. We call this step pre-pruning \bb{how about to call it shaping?} and
